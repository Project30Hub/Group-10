...
> **Note:** The version below has one fix from the original repo file — the docstrings used escaped quotes (`\"\"\"`) which is invalid Python syntax and would crash with a `SyntaxError`. They've been changed to normal triple quotes (`"""`).
